# Explorer選択動作 調査記録（v1.0開発時の履歴資料）

> **Status:** Historical design research from the v1.0 development cycle.
>
> RC1〜RC10でExplorerの選択動作を調査した当時の記録です。公開v1.0では安定性を優先してnative WPF Extended selectionへロールバックしました。その後、v1.1.0でPrototype 27を基準とする再設計版rubber-band selectionが正式採用されています。現在仕様は`docs/RUBBER_BAND_SELECTION_SPEC_JA.md`を参照してください。
