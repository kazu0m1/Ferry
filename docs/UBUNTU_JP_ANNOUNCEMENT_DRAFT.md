# Ubuntu日本語コミュニティ向け 紹介文ドラフト — Ferry v1.1.0
...
一方でWindowsへ戻ると、特に「フォルダー内のアイテム数がすぐ見えること」「現在フォルダー以下を自然に検索できること」「複数ファイルを手早く扱えること」が恋しくなりました。

そこで、Nautilusそのものを移植するのではなく、その使い心地から着想を得たWindows 11向けの軽量ファイルマネージャー **Ferry** を作りました。

現在のFerry v1.1.0では、次のような機能があります。
...
- ZIP圧縮・展開
- Explorerに近いrubber-band（矩形）選択
- `Ctrl` / `Shift` / `Ctrl+Shift`を使った複数選択とSelection Anchor表示
- 選択中のedge autoscroll
- List / Grid切替時の選択状態維持
- `Ctrl+L`でLocation Boxを開き、`Esc`等でBreadcrumbへ戻る操作
